...
class PaTagConfig(AppConfig):
    ...
    name = 'pa_tag'
    verbose_name = 'Tags'
    description = '''The application includes a tagging feature that allows users to assign tags to contacts, providing a way to categorize and organize them more efficiently. Tags can be created and deleted, but the deletion functionality is unique. Specifically, users cannot delete a tag if it is currently being used by any contact, ensuring that no data becomes disorganized or improperly categorized. This rule enforces data consistency and prevents accidental removal of important tags.
